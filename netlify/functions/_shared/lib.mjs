...
// Derive two independent app-only cryptographic keys from the existing
// high-entropy Supabase service-role secret. This keeps setup simple while
// still separating OAuth-state signing from refresh-token encryption.
function derivedKey(label){
  return crypto
    .createHmac('sha256',env('SUPABASE_SERVICE_ROLE_KEY'))
    .update(`my-life:${label}:v1`)
    .digest();
}
...
export function createState(userId){
  ...
  const sig=crypto.createHmac('sha256',derivedKey('oauth-state')).update(payload).digest('base64url');
  return `${payload}.${sig}`;
}
...
export function verifyState(state){
  ...
  const expected=crypto.createHmac('sha256',derivedKey('oauth-state')).update(payload).digest('base64url');
...
function key(){
  return derivedKey('google-refresh-token');
}
